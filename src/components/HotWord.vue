...
const getHotWord = async () => {
  ...
  try {
    const res: ApiResponse = await fetch(`${apiBase}/trendanalysis/get?year=${selectedYear.value}&month=${selectedMonth.value}`, {
    ...
    if (data.StatusCode === 200) {
      load.value = false
      console.log(data.sortedReturnResult)
...
<template>
  <div class="flex justify-between py-5 items-center">
    ...
    <div class="flex p-4 gap-4 items-center h-[40px]">
      ...
      <button class="border h-[40px] px-4 bg-blue-900 text-white rounded" @click="getHotWord">查詢</button>
        ...
        <div class="py-2 align-middle inline-block min-w-full sm:px-6 lg:px-8">
          <div class="shadow overflow-hidden border-b border-gray-200 sm:rounded-lg">
            <table class="min-w-full divide-y divide-gray-200">
              ...
              <tbody class="bg-white divide-y divide-gray-200" v-for="(data, index) in datas" :key="index">
